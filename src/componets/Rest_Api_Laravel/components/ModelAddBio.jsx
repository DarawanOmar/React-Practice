...
const ModelAddBio = ({setShowAddBio, setReload}) => {
    ...
    const submitFomr = async (e) => {
        e.preventDefault();
        const response = await axios.post("http://localhost:8000/api/addbio",{bio},{headers: {
            ...
            <div className=" rounded-md bg-gradient-to-r from-indigo-50  via-indigo-200  to-indigo-50">
                    ...
                    <form onSubmit={submitFomr} className='px-10'>
                        <div className="bg-white rounded-md p-2 mt-2 ">
                            <textarea onChange={(e) => setBio(e.target.value)} className='rounded-md text-center placeholder:text-center placeholder:text-gray-400  focus:outline-none' placeholder='Bio' cols="30" rows="2"></textarea>
                        </div>
