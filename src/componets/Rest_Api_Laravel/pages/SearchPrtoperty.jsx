...
const SearchPrtoperty = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const resopnse = await axios.get(
        `http://localhost:8000/api/properties?city_id=${city_id}`,
  ...
  const handleFilter = async () => {
    const resopnse = await axios.get(
      `http://localhost:8000/api/properties?city_id=${city_id}&area[0]=${valuesArea[0]}&area[1]=${valuesArea[1]}&price[0]=${valuesPrice[0]}&price[1]=${valuesPrice[1]}`,
      { headers: { Accept: "application/json" } }
    );
    setPisplayProperties(resopnse.data.properties.data);
  };
  ...
  const searchProperty = (name) => {
    const filterCity = properties.filter((pro) => {
      return (
        pro.catigorey.name.toLowerCase().includes(name.toLowerCase()) ||
        pro.title.toLowerCase().includes(name.toLowerCase())
      );
    });
  ...
  return (
    <div
      className={
        isDark
          ? " bg-black text-whitew-full object-cover duration-500"
      ...
      >
        <div className="flex justify-between items-center p-2 px-4 ">
          <div className="flex items-center space-x-3">
            <Link
              to="/properties"
              className="md:hidden text-xl text-indigo-500"
            >
              <IoIosArrowBack />
            </Link>
          ...
          {displayProperties.length > 0 ? (
            ...
            <>
              <h1 className="text-2xl text- font-bold">
                <span className="text-red-600 ">{search}</span> Not Found
              </h1>
            </>
          )}
        </div>
