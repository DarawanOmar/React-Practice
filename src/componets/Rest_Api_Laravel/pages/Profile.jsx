...
const Profile = () => {
    ...
    const fettchDataUser = async () => {
      try {
        const data = await axios.get(
          "http://localhost:8000/api/profile/properties",
          {
            headers: {
  ...
  const showNotifiDelete = () => [toast.error("Property Delete SuccessFully")];
      ...
      >
        ...
        <div className="flex justify-between md:justify-end items-center p-3">
          <div className="flex md:hidden items-center">
            <Link to="/setting" className="text-xl text-indigo-500">
              <IoIosArrowBack />
            </Link>
            <h1 className="text-xl font-bold  ">
              <Link to="/setting"> Own Profile</Link>
            </h1>
          </div>
          <span
            onClick={() => setShowEdit((prev) => !prev)}
            className="text-2xl md:text-3xl cursor-pointer rounded-md"
          >
            {" "}
            ...
            <div className="">
              ...
              <div className="relative">
                {user?.cover ? (
                  <>
                    <img
                      className="max-h-[200px] md:max-h-[390px] w-full object-cover"
                      src={`http://localhost:8000/upload/users/${user.cover}`}
                      alt="buling"
                    />
                  </>
                ) : (
                  <>
                    <img
                      className="max-h-[200px] md:max-h-[390px] w-full object-cover"
                      src="https://img.freepik.com/free-photo/top-view-wooden-frame-with-copy-space_23-2148553336.jpg?size=626&ext=jpg&uid=R76012333&ga=GA1.2.656506428.1687210625&semt=sph"
                      alt="buling"
                    />
                  </>
                )}
                <div className="absolute top-[50%] md:top-[60%] left-[35%] md:left-[40%] lg:left-[45%] border-[6px] rounded-full border-white">
                  {user?.image ? (
                    <>
                      <img
                        className="w-28 h-28 md:w-44 md:h-44  rounded-full object-cover"
                        src={user.photo}
                        alt=""
                      />
                    </>
                  ...
                  className={
                    showEdit
                      ? `absolute top-0 right-0 ${
                        ...
                        } flex flex-col justify-center items-center p-3 rounded-md translate-x-0 ease-in-out duration-500 `
                      : "-translate-x-2 text-white ease-in duration-500 -z-10 absolute top-7 right-2"
                  }
                >
                  <button
                    onClick={() => {
                  ...
                  <button
                    ...
                    className="pt-1 border-b-2 border-black w-fullakshlerin@example.com"
                  >
                    Add Bio
                  </button>
                </div>
              ...
              <div className="text-center mt-3">
                {user.bio ? (
                  <h1 className="px-2 md:text-2xl">{user.bio}</h1>
                ) : null}
              </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6  p-3">
              {propertiesUser.map((task) => {
                return (
                  <PropertiesUserProfile
                    key={task.id}
                    {...task}
                    loader={loader}
                    deletePropery={deletePropery}
                    setId={setId}
                    setReload={setReload}
                    showNotifiDelete={showNotifiDelete}
                  />
                );
              })}
            </div>
            ...
            {!loader && (
              <>
                ...
                <h1 className="mb-4 font-bold ">
                  You Don't Have Property Yet!
                </h1>
                <Link
                  to="/addproperty"
                  className=" bg-gradient-to-r from-indigo-500  via-indigo-700  to-indigo-500 px-6 py-2 rounded-md text-white"
                >
                  Add Property
                </Link>
              </>
        ...
        {showAddImageCover && (
          <ModelAddImageCover
            setShowAddImageCover={setShowAddImageCover}
            setReload={setReload}
          />
        )}
        {showAddImageProfile && (
          <ModelAddImageProfile
            setShowAddImageProfile={setShowAddImageProfile}
            setReload={setReload}
          />
        )}
        {showAddBio && (
          <ModelAddBio setShowAddBio={setShowAddBio} setReload={setReload} />
        )}
        <ToastContainer
          position="top-right"
          theme="light"
