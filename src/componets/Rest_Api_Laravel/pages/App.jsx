...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setpropertiesPriceDesc((prevProperties) => [
        ...prevProperties,
        ...propertiesPriceDesc.data.data.data,
      ]);
    };
    fetchData();
    setTimeout(() => {
  ...
  useEffect(() => {
    const handleFilterCatigorey = async () => {
      const response = await axios.get(
        `http://localhost:8000/api/properties?catigorey_id=${cati}&page=${page}`
      );
      setpropertiesCatigorey((prev) => [
        ...prev,
  ...
  const searachInput = async (name) => {
    ...
    const filterCity = city.data.filter((city) => {
      return (
        name && city && city.name.toLowerCase().includes(name.toLowerCase())
      );
    });
  ...
  const handleFilter = async () => {
    const response = await axios.get(
      `http://localhost:8000/api/filterbetween?area[0]=${valuesArea[0]}&area[1]=${valuesArea[1]}&price[0]=${valuesPrice[0]}&price[1]=${valuesPrice[1]}`
    );
  ...
  return (
    ...
    >
      <div id="1" className={`max-w-7xl mx-auto relative font-realEstate2`}>
        {/* User Name & Profile */}
        ...
        <div className="flex flex-col md:flex-row-reverse md:justify-between md:items-center p-4 relative">
          ...
          <div className="flex justify-between my-4 md:space-x-6">
            <div
              className={`flex text-black bg-white items-center rounded-2xl mr-2 w-full`}
            >
              ...
              <input
                ...
                className="p-3 bg-transparent focus:outline-none w-full rounded-lg  placeholder:text-gray-400 shadow-sm"
                type="text"
            ...
            <div>
              <div className="flex items-center space-x-2 bg-indigo-600  rounded-xl px-4 py-[11px] ">
                ...
                <h1 className="text-white">
                  <button onClick={filterButton}>Filters</button>
                </h1>
              </div>
            </div>
          </div>
                ...
                {cities.map((city) => (
                  <div key={city.id}>
                    <h1
                      ...
                      }  rounded-b-xl md:rounded-none p-2 `}
                    >
          ...
          <div className="flex justify-between items-center  mt-6 mb-8 md:space-x-10 lg:space-x-20">
            ...
            <a href="#2">
              <button
                ...
                className="bg-white flex justify-center items-center rounded-lg p-3"
              >
                <img className="w-8 h-8" src={apet} alt="" />
              </button>
            </a>
            <a href="#2">
              <button
                ...
                className="bg-white flex justify-center items-center rounded-lg p-3"
              >
                <img className="w-8 h-8" src={landd} alt="" />
              </button>
            </a>
            <a href="#2">
              <button
                ...
                className="bg-white flex justify-center items-center rounded-lg p-3"
              >
                <img className="w-8 h-8" src={house} alt="" />
              </button>
            </a>
          </div>
        </div>
          ...
          {filter && (
            <>
              ...
              <div className="  mt-16 flex justify-between items-center">
                ...
                <a
                  href="#2"
                  className="text-center  text-lg bg-blue-700 text-white py-[5px] rounded-md px-6 "
                >
                  <button
                    onClick={() => {
                      handleFilter();
        ...
        {propertiesPriceDesc.length > 0 || propertiesCatigorey.length > 0 ? (
          <>
            <div
              id="2"
              className="grid grid-cols-1  md:grid-cols-2 lg:grid-cols-3 gap-6  p-4 scroll-mt-20 "
            >
              {propertiesCatigorey.length > 0
                ? propertiesCatigorey.map((item, index) => {
                    return <List key={index} {...item} loader={loader} />;
                  })
        ...
        <div className="text-center pb-20 min-h-screen">
          {propertiesPriceDesc.length < 100 && (
            <button
              onClick={seeMore}
              className="btn-action mb-10 hover:opacity-60 "
            >
              See More
            </button>
          )}
