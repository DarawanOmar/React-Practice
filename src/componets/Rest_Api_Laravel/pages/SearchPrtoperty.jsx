...
const SearchPrtoperty = () => {
  ...
  const handleFilter = async () => {
    ...
    setPisplayProperties(response.data.properties.data)
}

  // Input Search
  const searchProperty =  (name) => {
    const filterCity = properties.filter(pro => { 
      return  pro.catigorey.name.toLowerCase().includes(name.toLowerCase())
    })
    ...
    <div className='bg-neutral-100 h-screen'>
      ...
      <div className="flex justify-between my-4 md:space-x-6 px-4">
          <div className="flex bg-white items-center rounded-2xl mr-2">
              ...
              <input value={search} onChange={(e) => handleSearch(e.target.value)} className='p-3 bg-white focus:outline-none  rounded-lg  placeholder:text-gray-400 shadow-sm' type="text" placeholder='Search Catigorey...' />
          </div>
          <div>
              <div className="flex items-center space-x-2 bg-blue-700 rounded-xl px-4 py-[11px] ">
                  ...
                  <button onClick={filterButton} className='text-white'>Filters</button>
              </div>
          </div>
      </div>

      <div className={filter ? 'bg-zinc-200 w-full h-[490px] fixed bottom-14 rounded-t-[40px] p-4  ease-in-out duration-500 z-10' : ' translate-y-72 ease-in-out duration-500'}>
            {filter && <>
