...
const Property = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const comments = await axios.get(
        `http://localhost:8000/api/propertyfavoraite/${id}`,
        {
          headers: {
      ...
      const checkadded = await axios.get(
        `http://localhost:8000/api/checkadded/${id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            Accept: "application/json",
  ...
  const addPropertyToFavorairte = async () => {
    const response = await axios.post(
      ...
      { property_id: id, addtofavoraite: "true" },
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          Accept: "application/json",
  ...
  const addCommentToProperty = async () => {
    const response = await axios.post(
      `http://localhost:8000/api/favoraiteaddcomment`,
      { property_id: id, comment: textComment },
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          Accept: "application/json",
    ...
    return (
      <div className={`${isDark ? "bg-black min-h-screen" : "bg-white"}`}>
        <div className="max-w-7xl mx-auto ">
          ...
          <div className="lex flex-col space-y-3 p-3">{<Tests />}</div>
        </div>
      </div>
    );
  }
      ...
      >
        ...
        <div className="flex justify-between items-center p-2 px-4">
          <div className="flex items-center space-x-3 p-1">
            <Link to="/home" className="text-xl text-indigo-500">
              <IoIosArrowBack />
            </Link>
        ...
        <div className="flex flex-col justify-center items-center mb-24 ">
          <h1 className="text-center font-bold mb-2 md:text-xl">
            You Can Write Your Opinion About This Proaperty
          </h1>
          <textarea
            onChange={(e) => setTextComment(e.target.value)}
            value={textComment}
            type="text"
            className="p-2 text-center rounded-md w-full bg-gradient-to-r from-indigo-50  via-indigo-200  to-indigo-50 focus:outline-none placeholder:text-center"
            placeholder="Comment..."
          />
          <button
            onClick={() => {
            ...
            className="bg-gradient-to-b from-indigo-500  via-indigo-700  to-indigo-500 px-6 py-2 mt-3 rounded-md text-white"
          >
            Post Comment
          </button>
        </div>
        {/* Comments */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-6 pb-20 md:pb-4">
          {comments.length > 0 && (
            <>
